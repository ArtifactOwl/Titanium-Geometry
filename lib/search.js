...
export function sortProducts(products, sortKey, priceOf = (p) => Number(p.price || 0)) {
  ...
  switch (sortKey) {
    ...
    default: {
      // "Featured" puts hand-picked pieces first, then everything else —
      // newest first within each band. Falling back to products.json order
      // buried new work at the very end of the shop, which is the opposite of
      // what a returning visitor wants to see.
      const newestFirst = (a, b) =>
        String(b.created || "").localeCompare(String(a.created || "")) ||
        String(b.itemId || "").localeCompare(String(a.itemId || ""));
      return list.sort(
        (a, b) => (b.featured ? 1 : 0) - (a.featured ? 1 : 0) || newestFirst(a, b)
      );
    }
  }
}
