...
export const SITE_URL = "https://titaniumgeometry.com";
...
/**
 * Whether a dated thing is still running. No date means it never expires.
 *
 * Evaluated whenever a price is worked out, in the browser as well as at build
 * time, so a sale that runs out stops applying on its own rather than waiting
 * for the next publish.
 */
export function stillLive(expires) {
  if (!expires) return true;
  // Date-only comparison, so a sale runs through the whole of its final day.
  return new Date(expires + "T23:59:59") >= new Date();
}
...
export function flagDiscountFor(product, basePrice) {
  ...
  for (const key of flags) {
    ...
    if (!rule || rule.active === false || !stillLive(rule.expires)) continue;
...
export function priceInfo(product) {
  const list = Number((product && product.originalPrice) || (product && product.price) || 0);
  let current = Number((product && product.price) || 0);
  // A sale with a date on it reverts by itself once that date passes.
  if (product && product.salePrice != null && !stillLive(product.saleExpires)) {
    current = list;
  }
...
const isExpired = (coupon) => !stillLive(coupon.expires);
